...
final SERVICES = [
  ...
  {
    'name': 'Quote after inspection',
    'icon': FontAwesomeIcons.clipboardCheck,
    'type': 'quote_after_inspection',
  },
  {
    'name': 'Add Notes',
...
class ApiService {
  ...
  static Future<Map<String, dynamic>> calculatePrice(
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to calculate price: ${response.statusCode}');
    }
  }
                ...
                onCreateRequest: () async {
                  try {
                    ...
                    if (provider.currentLocationCoords == null ||
                      ...
                      SnackBarUtil.showWarning(context, 'Location or service type not available');
                    ...
                    if (mounted) {
                      SnackBarUtil.showError(context, 'Error creating service request: $e');
                    }
                  } finally {
                    if (mounted) {
                      provider.setLoading(false);
...
class _MapSectionState extends State<MapSection> {
  ...
  Future<void> _checkLocationServices() async {
    ...
    Provider.of<MechanicProvider>(
      context,
      listen: false,
    ).setLocationServiceStatus(serviceEnabled);
  ...
  Future<void> _getCurrentPosition() async {
    ...
    try {
      ...
      if (!mounted) return;
      SnackBarUtil.showError(context, 'Error getting location: $e');
    } finally {
      if (mounted) {
        provider.setLoading(false);
      }
...
class _ServicesSelectionSectionState extends State<ServicesSelectionSection> {
  ...
  Future<void> _calculatePrice() async {
    ...
    if (!provider.canRequestService) {
      if (!provider.locationServiceEnabled) {
        SnackBarUtil.showWarning(context, 'Please enable location services to request a mechanic');
        return;
      }

      if (!provider.hasValidServiceSelected) {
        SnackBarUtil.showWarning(context, 'Please select a service from the first 5 options');
        return;
      }

      if (provider.currentLocationCoords == null) {
        SnackBarUtil.showInfo(context, 'Please wait while we get your location');
        return;
      }
    }
    ...
    try {
      ...
      final response = await ApiService.calculatePrice(
        serviceType,
        provider.authToken ?? '',
      );
      ...
      if (mounted) {
        SnackBarUtil.showError(context, 'Error calculating price: $e');
      }
    } finally {
      if (mounted) {
        provider.setLoading(false);
          ...
          if (!_keyboardVisible)
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: provider.canRequestService
                      ? Colors.green
                      : Colors.grey,
                  shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(12),
