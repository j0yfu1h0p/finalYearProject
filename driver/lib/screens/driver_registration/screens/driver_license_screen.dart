// screens/driver_license_screen.dart
import 'dart:io';

import 'package:flutter/material.dart';
import 'package:provider/provider.dart';
import 'package:image_picker/image_picker.dart';
import '../../../providers/driver_registration_provider.dart';

class DriverLicenseScreen extends StatefulWidget {
  final GlobalKey<FormState> formKey;

  DriverLicenseScreen({required this.formKey});

  @override
  _DriverLicenseScreenState createState() => _DriverLicenseScreenState();
}

class _DriverLicenseScreenState extends State<DriverLicenseScreen> {
  final ImagePicker _picker = ImagePicker();
  TextEditingController _expiryController = TextEditingController();

  @override
  Widget build(BuildContext context) {
    final provider = Provider.of<RegistrationProvider>(context);

    return Padding(
      padding: EdgeInsets.all(16),
      child: Form(
        key: widget.formKey,
        child: SingleChildScrollView(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(
                'Driver\'s License',
                style: TextStyle(
                  fontSize: 22,
                  fontWeight: FontWeight.bold,
                  color: Colors.black,
                  fontFamily: "UberMove",
                ),
              ),
              SizedBox(height: 16),
              Text(
                'Please provide your license details',
                style: TextStyle(
                  color: Colors.grey[600],
                  fontSize: 14,
                  fontFamily: "UberMove",
                ),
              ),
              SizedBox(height: 24),
              Text(
                'License Number',
                style: TextStyle(
                  fontWeight: FontWeight.bold,
                  color: Colors.black,
                  fontFamily: "UberMove",
                ),
              ),
              SizedBox(height: 8),
              TextFormField(
                decoration: InputDecoration(
                  hintText: 'Enter license number',
                  hintStyle: TextStyle(fontFamily: "UberMove"),
                  errorStyle: TextStyle(fontFamily: "UberMove"),
                ),
                validator: (value) {
                  if (value == null || value.isEmpty) {
                    return 'Please enter your license number';
                  }
                  return null;
                },
                onSaved: (value) => provider.updateLicenseNumber(value!),
                initialValue: provider.data.licenseNumber,
                style: TextStyle(color: Colors.black),
              ),
              SizedBox(height: 16),
              Text(
                'License Expiry Date',
                style: TextStyle(
                  fontWeight: FontWeight.bold,
                  color: Colors.black,
                  fontFamily: "UberMove",
                ),
              ),
              SizedBox(height: 8),
              TextFormField(
                decoration: InputDecoration(
                  hintText: 'Select expiry date',
                  suffixIcon: Icon(
                    Icons.calendar_today,
                    size: 20,
                    color: Colors.grey[600],
                  ),
                ),
                readOnly: true,
                onTap: () => _selectExpiryDate(context, provider),
                controller: _expiryController,
                validator: (value) {
                  if (provider.data.licenseExpiryDate == null) {
                    return 'Please select license expiry date';
                  }
                  return null;
                },
                style: TextStyle(color: Colors.black),
              ),
              SizedBox(height: 32),
              Text(
                'License Photo',
                style: TextStyle(
                  fontWeight: FontWeight.bold,
                  color: Colors.black,
                  fontFamily: "UberMove",
                ),
              ),
              SizedBox(height: 8),
              Text(
                'Clear photo with visible details',
                style: TextStyle(color: Colors.grey[600], fontSize: 12),
              ),
              SizedBox(height: 8),
              _buildImageUploadCard(
                title: 'Tap to capture license photo',
                image: provider.data.licensePhoto,
                onTap: () => _pickImage(provider),
              ),
            ],
          ),
        ),
      ),
    );
  }

  Widget _buildImageUploadCard({
    required String title,
    File? image,
    required VoidCallback onTap,
  }) {
    return Card(
      elevation: 1,
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(12),
        side: BorderSide(color: Colors.grey[300]!),
      ),
      child: InkWell(
        onTap: onTap,
        borderRadius: BorderRadius.circular(12),
        child: Container(
          height: 180,
          width: double.infinity,
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              if (image != null)
                ClipRRect(
                  borderRadius: BorderRadius.circular(12),
                  child: Image.file(
                    image,
                    fit: BoxFit.cover,
                    width: double.infinity,
                    height: 180,
                  ),
                )
              else
                Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Container(
                      width: 60,
                      height: 60,
                      decoration: BoxDecoration(
                        shape: BoxShape.circle,
                        color: Colors.grey[100],
                      ),
                      child: Icon(
                        Icons.camera_alt,
                        size: 30,
                        color: Colors.grey[600],
                      ),
                    ),
                    SizedBox(height: 16),
                    Text(
                      title,
                      style: TextStyle(
                        color: Colors.grey[600],
                        fontSize: 14,
                        fontFamily: "UberMove",
                      ),
                    ),
                  ],
                ),
            ],
          ),
        ),
      ),
    );
  }

  Future<void> _pickImage(RegistrationProvider provider) async {
    final XFile? image = await _picker.pickImage(source: ImageSource.camera);
    if (image != null) {
      provider.updateLicensePhoto(File(image.path));
    }
  }

  Future<void> _selectExpiryDate(
    BuildContext context,
    RegistrationProvider provider,
  ) async {
    final DateTime? picked = await showDatePicker(
      context: context,
      initialDate: DateTime.now().add(Duration(days: 365)),
      firstDate: DateTime.now(),
      lastDate: DateTime.now().add(Duration(days: 3650)),
      builder: (context, child) {
        return Theme(
          data: ThemeData.light().copyWith(
            colorScheme: ColorScheme.light(
              primary: Colors.green,
              onPrimary: Colors.white,
              surface: Colors.white,
              onSurface: Colors.black,
            ),
            dialogBackgroundColor: Colors.white,
          ),
          child: child!,
        );
      },
    );
    if (picked != null) {
      provider.updateLicenseExpiryDate(picked);
      _expiryController.text = "${picked.day}/${picked.month}/${picked.year}";
    }
  }
}
