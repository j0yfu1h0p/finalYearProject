...
exports.searchDrivers = async (req, res) => {
    try {
        ...
        const drivers = await Driver.find(query)
            .select(
                'phoneNumber personal_info.first_name personal_info.last_name personal_info.registration_status vehicles personal_info.registration_date rating ratingCount'
            )
...
exports.searchMechanics = async (req, res) => {
    try {
        ...
        const mechanics = await Mechanic.find(query)
            .select('phoneNumber personName shopName registrationStatus servicesOffered createdAt rating ratingCount')
            .sort({ createdAt: -1 })
            .skip((page - 1) * limit)
            .limit(parseInt(limit))
            .lean();
